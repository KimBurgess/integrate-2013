...
//#INCLUDE 'RmsSchedulingEventListener';	// commented out by DV - causes issues
...
(* Args:  SINTEGER dayOfTheMonth - Day of the month, 1 - 31*)
(*        LONG locationId - location ID                    *)
(*                                                         *)
(* Desc:  Query monthly booking summary for specified day  *)
(*        of the month and location                        *)
(*                                                         *)
(*        If location ID is less than 1, the default       *)
(*        location will be used.                           *)
(*                                                         *)
...
(*        0 if call was unsuccessful                       *)
(***********************************************************)
DEFINE_FUNCTION CHAR RmsBookingsSummariesDailyRequest(SINTEGER dayOfTheMonth, LONG locationId)
{
  STACK_VAR CHAR rmsCommand[RMS_MAX_CMD_LEN];

  if(dayOfTheMonth < 1 || dayOfTheMonth > 31)
  {
    SEND_STRING 0, '>>>> RMS API ERROR <RmsBookingsSummariesDailyRequest> :: day of the month should be in the range of 1 - 31';
    RETURN FALSE;
  }

  // create send command
  rmsCommand = RmsPackCmdHeader(RMS_COMMAND_SCHEDULING_BOOKINGS_SUMMARIES_DAILY_REQUEST);
  rmsCommand = RmsPackCmdParam(rmsCommand,ITOA(dayOfTheMonth));
  if(locationId > 0)
  {
    rmsCommand = RmsPackCmdParam(rmsCommand,ITOA(locationId));
  }
