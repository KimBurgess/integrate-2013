...
{
  STACK_VAR RmsAssetSourceUsage sourceAsset;
            INTEGER index;
            FLOAT   usageHours
  ...
  IF(index > 0)
  {
    // get instance of source asset
    sourceAsset = sourceAssets[index];
    ...
    IF(newAssetRegistration)
    {
      ...
      DEBUG("'registering source usage parameter for asset [',assetClientKey,']'");

      usageHours = sourceAsset.usageMinutes / 60;
...
(***********************************************************)
(* Name:  HasSourceAsset                                   *)
(* Args:  assetClientKey - unique asset key identifier     *)
(*                                                         *)
...
(***********************************************************)
DEFINE_FUNCTION CHAR HasSourceAsset(CHAR assetClientKey[])
{
  IF(GetSourceAssetIndex(assetClientKey) > 0)
  {
    RETURN TRUE;
  }
  ELSE
...
(***********************************************************)
DEFINE_FUNCTION INTEGER GetSourceAssetIndex(CHAR assetClientKey[])
{
  ...
  {
    IF(assetClientKey == sourceAssets[index].assetClientKey)
    {
      RETURN index;
    }
  }
...
DEFINE_FUNCTION INTEGER ResetAllActiveSources()
{
  ...
  FOR(index = 1; index <= maxAssignedSourceAssetIndex; index++)
  {
    ...
    {
      DEBUG("'deactivating and resetting source asset #',ITOA(index),' [',sourceAssets[index].assetClientKey,']'");
...
DEFINE_FUNCTION CHAR SendSourceAssetUsageToRms(RmsAssetSourceUsage sourceAsset)
{
     ...
     sourceAsset.usageMinutes > 0)
  {
       ...
       [vdvRMS,RMS_CHANNEL_ASSETS_REGISTER])
    {
      ...
      DEBUG("'sending accumulated source usage for asset [',sourceAsset.assetClientKey,']  / increment by [',FTOA(usageHours),'] hours'");
...
DEFINE_FUNCTION DeactivateMutuallyExclusiveSources(INTEGER skipSourceIndex)
{
  ...
  FOR(index = 1; index <= maxAssignedSourceAssetIndex; index++)
  {
   ...
   sourceAssets[index].activated)
      {
        DEBUG("'deactivating mutually exclusive source asset #',ITOA(index),' [',sourceAssets[index].assetClientKey,']'");
...
DEFINE_FUNCTION DeactivateAllSources()
{
  ...
  FOR(index = 1; index <= maxAssignedSourceAssetIndex; index++)
  {
    IF(sourceAssets[index].activated)
    {
      DEBUG("'deactivating source asset #',ITOA(index),' [',sourceAssets[index].assetClientKey,']'");
...
DEFINE_FUNCTION SelectSource(INTEGER index, CHAR selected)
{
  ...
  IF(index > 0 && index <= maxAssignedSourceAssetIndex)
  {
    ...
    {
      DEBUG("'select source asset #',ITOA(index),' [',sourceAssets[index].assetClientKey,'] / activated: [',RmsBooleanString(selected),']'");
  ...
  COMMAND:
  {
    ...
    SELECT
    {
      ...
      ACTIVE (sourceCmdHeader == RMS_SOURCE_USAGE_COMMAND_ASSIGN_SOURCE):
      {
        ...
        sourceCmdParam2 = RmsParseCmdParam(DATA.TEXT); // source asset client key
        ...
        IF(LENGTH_STRING(sourceCmdParam1) > 0 && LENGTH_STRING(sourceCmdParam2) > 0)
        {
          ...
          IF(index > 0 && index <= MAX_SOURCE_ASSETS)
          {
            ...
            sourceAssets[index].assetClientKey = sourceCmdParam2;
            sourceAssets[index].usageMinutes = 0;
            ...
            IF(sourceAssets[index].mutuallyExclusive)
      {
        DEBUG("'source asset [',sourceAssets[index].assetClientKey,'] assigned to source index [',sourceCmdParam1,'], source is mutually exclusive.'");
      }
      ELSE
      {
        DEBUG("'source asset [',sourceAssets[index].assetClientKey,'] assigned to source index [',sourceCmdParam1,'], source is not mutually exclusive.'");
      }
          }
        }
      }
...
TIMELINE_EVENT[TL_MONITOR]
{
  ...
  FOR(index = 1; index <= maxAssignedSourceAssetIndex; index++)
  {
       ...
       sourceAssets[index].activated)
    {
      ...
      DEBUG("'source asset #',ITOA(index),' [',sourceAssets[index].assetClientKey,'] is active, incrementing minute count to [',ITOA(sourceAssets[index].usageMinutes),']'");
